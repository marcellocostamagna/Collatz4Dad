...
# User Inputs for Sequence 1
st.sidebar.header("Settings for Sequence 1", divider=True)
number1_input = st.sidebar.text_input("Starting Number ", "7", key="series1_number")
max_steps1 = st.sidebar.number_input("Max Steps (0 for full sequence) ", min_value=0, value=0, step=1, key="series1_steps")
...
# User Inputs for Sequence 2
st.sidebar.header("Settings for Sequence 2", divider=True)
...
try:
    ...
    st.sidebar.error("Invalid input for Starting Number (Sequence 1). Please enter a valid integer.")
    number1 = None

try:
    number2 = int(number2_input)
except ValueError:
    st.sidebar.error("Invalid input for Starting Number (Sequence 2). Please enter a valid integer.")
    number2 = None

# Button to Trigger Calculation for Series 1
if st.button("Calculate Sequence 1") and number1 is not None:
...
if st.session_state['sequence1']:
    ...
    with col3:
        ...
        if st.button("Info", key="info_seq_1"):
...
if st.button("Calculate Sequence 2") and number2 is not None:
...
if st.session_state['sequence2']:
    ...
    with col6:
        ...
        if st.button("Info", key="info_seq_2"):
